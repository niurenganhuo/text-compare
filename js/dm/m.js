...
(function (window, document, jQuery, CodeMirror) {
    ...
    function jqpd() {
        ...
        console.log(rLsajvwEd2)
        // if (ND1 == "\x68\x74\x74\x70\x3a\x2f\x2f\x77\x77\x77\x2e\x6a\x71\x32\x32\x2e\x63\x6f\x6d" || rLsajvwEd2 == "\x66\x69\x6c\x65")) {
   ...
   jqpd();
    Mgly.DiffParser = function (diff) {
        ...
        for (var i = 0; i < diff_lines.length; ++i) {
            ...
            if (fr.length == 1) change['lhs-line-to'] = fr[0] - 1; else change['lhs-line-to'] = fr[1] - 1;
            var to = test[3].split(',');
            change['rhs-line-from'] = to[0] - 1;
            if (to.length == 1) change['rhs-line-to'] = to[0] - 1; else change['rhs-line-to'] = to[1] - 1;
            change['op'] = test[2];
        ...
        clear: function () {
            ...
            for (var i = 0; i < changes.length; ++i) {
                ...
                if (change.op != 'a') {
                    ...
                    if (change.op == 'd') lchange += ' '; else if (li > 0 && change.op == 'c') li += 1;
                ...
                if (change.op != 'd') {
                    ...
                    if (change.op == 'a') rchange += ' '; else if (li > 0 && change.op == 'c') li += 1;
    ...
    Mgly.diff = function (lhs, rhs, options) {
        ...
        this.codeify = new Mgly.CodeifyText({lhs: lhs, rhs: rhs, options: opts});
    ...
    jQuery.extend(Mgly.diff.prototype, {
        changes: function () {
            ...
            for (var index = 0; index < this.items.length; ++index) {
                ...
                var rhs_str = '';
                var change = 'c';
                if (item.lhs_deleted_count == 0 && item.rhs_inserted_count > 0) change = 'a'; else if (item.lhs_deleted_count > 0 && item.rhs_inserted_count == 0) change = 'd';
                if (item.lhs_deleted_count == 1) lhs_str = item.lhs_start + 1; else if (item.lhs_deleted_count == 0) lhs_str = item.lhs_start; else lhs_str = (item.lhs_start + 1) + ',' + (item.lhs_start + item.lhs_deleted_count);
                if (item.rhs_inserted_count == 1) rhs_str = item.rhs_start + 1; else if (item.rhs_inserted_count == 0) rhs_str = item.rhs_start; else rhs_str = (item.rhs_start + 1) + ',' + (item.rhs_start + item.rhs_inserted_count);
            ...
            for (d = 0; d <= maxd; ++d) {
                ...
                for (k = kup - d; k <= kup + d; k += 2) {
                    if (k == kup + d) {
                        ...
                        if ((k > kup - d) && (vector_u[offset_up + k - 1] < x)) x = vector_u[offset_up + k - 1]
                    }
                    y = x - k;
            ...
            while (lhs_line < lhs_ctx.codes.length || rhs_line < rhs_ctx.codes.length) {
                if ((lhs_line < lhs_ctx.codes.length) && (!lhs_ctx.modified[lhs_line]) && (rhs_line < rhs_ctx.codes.length) && (!rhs_ctx.modified[rhs_line])) {
                    ...
                    lhs_start = lhs_line;
                    rhs_start = rhs_line;
                    while (lhs_line < lhs_ctx.codes.length && (rhs_line >= rhs_ctx.codes.length || lhs_ctx.modified[lhs_line])) lhs_line++;
                    while (rhs_line < rhs_ctx.codes.length && (lhs_line >= lhs_ctx.codes.length || rhs_ctx.modified[rhs_line])) rhs_line++;
    ...
    jQuery.extend(Mgly.mergely.prototype, {
        name: 'mergely', init: function (el, options) {
    ...
    jQuery.extend(Mgly.CodeMirrorDiffView.prototype, {
        init: function (el, options) {
            this.settings = {
                ...
                resize: function (init) {
                    ...
                    if (self.css('display') == 'none') {
                        if (this.fadein != false) self.fadeIn(this.fadein); else self.show();
            ...
            var le = this.editor[this.id + '-lhs'];
            var re = this.editor[this.id + '-rhs'];
            if (side == 'lhs' && !this.lhs_cmsettings.readOnly) le.setValue(re.getValue()); else if (!this.rhs_cmsettings.readOnly) re.setValue(le.getValue())
        }, get: function (side) {
            ...
            var re = this.editor[this.id + '-rhs'];
            var editor;
            if (side == 'lhs') editor = le; else editor = re;
            ...
            this.editor[this.id + '-lhs'].on('change', function () {
                if (self.settings.autoupdate) self._changing(self.id + '-lhs', self.id + '-rhs')
            });
            ...
            if (this.em_height == null) {
                if (!this._set_top_offset(editor_name1)) return;
            ...
            for (var i = 0; i < changes.length; ++i) {
                ...
                if (this.editor[editor_name1].getOption('lineWrapping') || this.editor[editor_name2].getOption('lineWrapping')) {
                    ...
                    ls = {top: lhschc.top + llf * this.em_height, bottom: lhschc.bottom + llf * this.em_height + 2};
                    le = {top: lhschc.top + llt * this.em_height, bottom: lhschc.bottom + llt * this.em_height + 2};
                    rs = {top: rhschc.top + rlf * this.em_height, bottom: rhschc.bottom + rlf * this.em_height + 2};
                    re = {top: rhschc.top + rlt * this.em_height, bottom: rhschc.bottom + rlt * this.em_height + 2}
                }
                if (change['op'] == 'a') {
                    if (rlf > 0) {
                        ls.top = ls.bottom;
            ...
            for (i = 0; this.settings.lcs && i < changes.length; ++i) {
                ...
                if (change['op'] == 'd') {
                    ...
                    if (to_ln) {
                        marktext.push([led, {line: from, ch: 0}, {
                            line: to,
                            ch: to_ln.text.length
                        }, {className: 'mergely ch d lhs'}])
                    }
                } else if (change['op'] == 'c') {
                    for (j = llf, k = rlf, p = 0; ((j >= 0) && (j <= llt)) || ((k >= 0) && (k <= rlt)); ++j, ++k) {
                        var lhs_line, rhs_line;
                        if (k + p > rlt) {
                            lhs_line = led.getLine(j);
                            marktext.push([led, {line: j, ch: 0}, {
                                line: j,
                                ch: lhs_line.length
                            }, {className: 'mergely ch d lhs'}]);
                            continue
